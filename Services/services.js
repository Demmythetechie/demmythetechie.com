import { boolContext } from "../src/App.js";
import { useContext, useEffect, useState } from "react";
import axios from "axios";

function Service () {
    const [ skills, updateSkills ] = useState(undefined);
    const [ services, updateServices ] = useState(undefined);
    useEffect(()=> {
        (async () => {
            await axios.get('http://localhost:4000/service')
              .then(function (response) {
                updateSkills(response.data[0]);
                updateServices(response.data[1]);
              })
            ...
            <section className="max-[1000px]:hidden w-[100%]">
                ...
                <div className="trans0 box-border opacity-0 w-[100%] aspect-[1/0.226] max-[1024px]:aspect-[1/0.25] relative overflow-hidden pt-[2%] pb-[1%] pl-[10%] pr-[10%] flex flex-row flex-wrap gap-x-[21%] gap-y-[11.5%] max-[1024px]:gap-x-[19.5%]">
                    {skills && skills.map((skill)=>(
                        <Skill cls={skill.anime} percentage={'0, 250'} name={skill.name} nameCls={'text-white text-[1.1vw] font-Ubuntu_Bold'} code={skill.code} codeCls={'text-white text-[1vw] font-Ubuntu_Bold'}/>
                    ))}
                </div>
            </section>
